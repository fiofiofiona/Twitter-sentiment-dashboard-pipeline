...
1. [Tweepy](https://www.tweepy.org/): Python library for requesting the official Twitter API. The standard search API has limitation of getting up to 100 tweets in each request, and user authentification can only make 180 requests per 15 minitue window.   
2. [Twint](https://github.com/twintproject/twint): Python library for fetching tweets without using API. 
